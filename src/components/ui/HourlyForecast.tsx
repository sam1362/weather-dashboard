...
export const HourlyForecast = ({ items, unit, loading, darkMode = true }: HourlyForecastProps) => {
  ...
  if (loading) {
    return (
      <section aria-label="Time for time" className="relative overflow-hidden rounded-3xl" style={{ minHeight: 280 }}>
        <div
        ...
        />
        <div className="relative z-10 p-6 md:p-8" style={{ minHeight: 280 }}>
          <div className="flex h-full flex-col gap-6">
            <div className="h-8 w-44 rounded-full bg-white/10 animate-pulse" />
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 min-h-[220px]">
  ...
  const sectionMin = fewItems ? 180 : 220
  const gridMin = fewItems ? 100 : 140

  return (
    <section aria-label="Time for time" className="relative overflow-hidden rounded-3xl" style={{ minHeight: sectionMin }}>
      <div
        className={`absolute inset-0 rounded-3xl backdrop-blur-xl ${
          darkMode
            ? 'bg-white/8 border border-white/10 shadow-[0_0_40px_rgba(0,0,0,0.35)]'
      ...
      <div className="relative z-10 p-6 md:p-8" style={{ minHeight: sectionMin }}>
        <div className="flex h-full flex-col gap-3">
          <header className="flex items-center justify-between min-h-[40px]">
            <div>
          ...
          {items.length === 0 ? (
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4" style={{ minHeight: gridMin }}>
              {items.map((item) => (
                <article
                  key={item.time}
                  className={`flex h-[100] sm:h-auto flex-col gap-3 rounded-2xl px-4 py-3 ${
                    darkMode
